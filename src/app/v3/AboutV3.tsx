  ...
  return (
    <section id="about" className="scroll-mt-24">
      <div className="grid grid-cols-1 md:grid-cols-[340px_1fr] gap-8 lg:gap-14 items-start">
        {/* Photo */}
        <div>
          <p className={`${mono} text-[10px] text-[#4D4D4D] mb-2.5`}>SEC-04 / Profile</p>
          ...
          <p className={`${mono} text-[10px] text-[#4D4D4D] mt-3`}>FIG. 02 — Portrait</p>
        </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap items-center gap-8 mt-8">
            <a href="#about" className={`${mono} text-[13px] font-bold text-[#FE6219] hover:text-[#F36A36] transition-colors`}>More about me →</a>
            <a href="https://www.linkedin.com/in/tom-carter-89403267/" target="_blank" rel="noopener noreferrer" className={`${mono} text-[13px] font-bold text-[#FE6219] hover:text-[#F36A36] transition-colors`}>LinkedIn ↗</a>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 mt-10 border border-[#4D4D4D]/40 rounded-[8px] bg-[#1F1F1F] overflow-hidden">
            {FACTS.map((f, i) => (
              <div key={f.label} className={`p-6 ${i > 0 ? "md:border-l" : ""} ${i >= 2 ? "border-t md:border-t-0" : ""} ${i % 2 === 1 ? "border-l md:border-l" : ""} border-[#4D4D4D]/40`}>
                <p className={`${mono} text-[10px] text-[#4D4D4D]`}>{f.label}</p>
